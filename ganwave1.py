...
# --- V8.6: Retuning Experiment ---
DATASET_PATH = "/home/tanmoyhazra/gan_project/data/"
SAVE_DIR = "/home/tanmoyhazra/gan_project/checkpoints_gw_retune"
SAMPLE_DIR = "/home/tanmoyhazra/gan_project/samples_gw_retune"
RESUME_CHECKPOINT_DIR = SAVE_DIR
...
# --- NEW: Linear KL Annealing ---
def kl_anneal(epoch, batch_idx, total_batches):
    progress = (epoch + batch_idx / total_batches) / 100 
    return min(1.0, progress)

# -----------------------------
# Data Preprocessing & Augmentation (Added Affine/Jitter)
# -----------------------------
...
# -----------------------------
# VAE-GAN Models (OLD Architecture)
# -----------------------------
...
# --- OLD Discriminator Class (to match checkpoint) ---
class Discriminator(nn.Module):
    def __init__(self, in_ch=3, base_ch=64, use_spectral_norm=True): # Added use_spectral_norm
        super().__init__()
        ...
        self.final_conv = spectral_norm(nn.Conv2d(base_ch * 8, 1, 1, 1, 0)) # Ends in Conv

    def forward(self, x):
        fmaps = []
        x = self.initial(x); x = self.initial_act(x)
        fmaps.append(x) # Add first fmap
...
# -----------------------------
# Utilities & Losses (Hinge Loss Adjusted)
# -----------------------------
def reparameterize(mu, logvar):
    logvar = torch.clamp(logvar, min=-10, max=10) # Added clamp for stability
    std = torch.exp(0.5 * logvar)
    eps = torch.randn_like(std)
    return mu + eps * std

def hinge_d_loss(real_out, fake_out):
    # Adjusted hinge loss from new code
...
# -----------------------------
# Eval Function (Kept V8.4 with fixes)
# -----------------------------
def evaluate_model(E, G, val_loader, device, num_fakes=1024, num_recon=500):
    ...
    class TensorDatasetWrapper(Dataset):
        def __init__(self, tensor):
            ...
            self.tensor = tensor # Expects (N, C, H, W), uint8

        def __len__(self):
            return self.tensor.shape[0]

        def __getitem__(self, index):
            img = self.tensor[index] # (C, H, W) uint8
            # Return tensor directly, in CHW format
    ...
    # Generate fakes
    fake_imgs = []
    batch_size_eval = 16 # Safe batch size
    with torch.no_grad():
    ...
    fid_metrics = calculate_metrics(
        ...
        fid=True, 
        isc=True, 
        batch_size=batch_size_eval # Pass batch size here
    )
    ...
    with torch.no_grad():
        for _ in range(num_recon_batches):
            ...
            for j in range(x.size(0)):
                # --- FIX: Use 3-channel SSIM/PSNR ---
                real_np_img = denorm(x[j].cpu().numpy().transpose(1, 2, 0)) # C,H,W -> H,W,C
                rec_np_img = denorm(rec[j].cpu().numpy().transpose(1, 2, 0)) # C,H,W -> H,W,C
                
                # Use multichannel=True and channel_axis=-1
...
def main(args):
    ...
    E = Encoder().to(DEVICE)
    G = Decoder().to(DEVICE)
    D = Discriminator(use_spectral_norm=True).to(DEVICE) # Use OLD Discriminator

    # --- NEW Optimizers with TTUR ---
    opt_EG = torch.optim.Adam(list(E.parameters()) + list(G.parameters()), lr=LR, betas=(BETA1, BETA2)) # EG uses LR
    opt_D = torch.optim.Adam(D.parameters(), lr=LR_D, betas=(BETA1, BETA2)) # D uses LR_D

    # Schedulers
    total_epochs_to_run_from_start = 100 # Set a total epoch count for schedulers
    ...
    if args.wandb:
        # --- NEW: Log hyperparameters to WandB ---
        config = vars(args)
        config.update({
            ...
            "latent_noise": 0.03, "kl_anneal_type": "linear", "run_type": "retune_from_epoch_50"
        })
        wandb.init(project="wavelet-vaegan-mri-retune", config=config)
    ...
    if args.resume:
        ...
        if checkpoint_files:
            ...
            start_epoch = checkpoint['epoch'] + 1 # Start from the next epoch (51 if loaded 50)
            
            print(f"Resumed. Starting from Epoch {start_epoch}")

            # --- NEW: Reset scheduler T_max for the *new* number of epochs ---
    ...
    for epoch in range(start_epoch, final_epoch):
        ...
        for i, x in enumerate(pbar):
            ...
            if torch.isnan(d_loss):
                ...
                opt_D.zero_grad() # Clear any bad grads
                continue
            d_loss.backward()
            opt_D.step()
            ...
            if accum_count % GRAD_ACCUM_STEPS == 0:
                ...
                opt_EG.step()
                opt_EG.zero_grad() # Zero E/G grads after step
                accum_count = 0
        ...
        # Handle last accum batch if needed
        if accum_count > 0:
            torch.nn.utils.clip_grad_norm_(list(E.parameters()) + list(G.parameters()), GRAD_CLIP)
            opt_EG.step()
            opt_EG.zero_grad() # Zero E/G grads after step
        ...
        # --- NEW: Diversity Log ---
        with torch.no_grad():
            G.eval() # Set G to eval mode for this
            ...
            G.train() # Set back to train mode
        ...
        # Step schedulers after logging LR
        scheduler_EG.step()
        scheduler_D.step()
        
        # Debug Pre-Tanh (No changes)
        with torch.no_grad():
            G.eval()
        ...
        # --- SAVE FIRST ---
        # --- NEW: Save every 10 epochs, but eval every 25 ---
        if (epoch + 1) % 10 == 0:
...
if __name__ == "__main__":
    ...
    # --- NEW: Changed default epochs to 30 ---
    parser.add_argument('--epochs', type=int, default=30, help="Total number of epochs to run *for this job*.")
